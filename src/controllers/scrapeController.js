...
//         send(res, "scrape_done", { totalLeads: leadsFound, timeTaken: elapsed + "s" });

//         // ── Auto-verify all newly scraped leads ───────────────
//         // Reuse the same browser + SSE connection — no second
//         // request needed. Verify every lead that has no ownerName yet.

//         send(res, "log", { message: "─────────────────────────────────────", type: "dim" });
//         send(res, "log", { message: "Starting Companies House verification...", type: "info" });
...
//                 failed++;
//                 send(res, "log", { message: `Verify error on ${searchName}: ${err.message}`, type: "error" });
